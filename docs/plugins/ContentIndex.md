...
This plugin emits both RSS and an XML sitemap for your site. The [[deploy/to-east.github.io/docs/features/RSS Feed]] allows users to subscribe to content on your site and the sitemap allows search engines to better index your site. The plugin also emits a `contentIndex.json` file which is used by dynamic frontend components like search and graph.
