...
- `fileData`: Any metadata [[deploy/to-east.github.io/docs/advanced/making plugins|plugins]] may have added to the current page.
...
If you need to create an `afterDOMLoaded` script that depends on _page specific_ elements that may change when navigating to a new page, you can listen for the `"nav"` event that gets fired whenever a page loads (which may happen on navigation if [[deploy/to-east.github.io/docs/features/SPA Routing]] is enabled).
...
You can also add the equivalent of a `beforeunload` event for [[deploy/to-east.github.io/docs/features/SPA Routing]] via the `prenav` event.
...
Then, you can use it like any other component in `quartz.layout.ts` via `Component.YourComponent()`. See the [[deploy/to-east.github.io/docs/configuration#Layout|layout]] section for more details.
